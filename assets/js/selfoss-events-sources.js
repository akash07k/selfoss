...
selfoss.events.sources = function() {
    ...
    $('.source-add').unbind('click').click(function() {
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                ...
                selfoss.ui.showError(selfoss.translate('error_add_source') + ' ' +
    ...
    $('.source-save').unbind('click').click(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // show saved text
                parent.find('.source-showparams').addClass('saved').html(selfoss.translate('source_saved'));
                window.setTimeout(function() {
                    parent.find('.source-showparams').removeClass('saved').html(selfoss.translate('source_edit'));
                }, 10000);
    ...
    $('.source-delete').unbind('click').click(function() {
        var answer = confirm(selfoss.translate('source_warn'));
        if (answer == false) {
            return;
        }
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                parent.find('.source-edit-delete').removeClass('loading');
                selfoss.ui.showError(selfoss.translate('error_delete_source') + ' ' + errorThrown);
            }
        });
    });
